...
if [ ! -d "DVP-ARDSLiteRoutingEngine" ]; then
	git clone -b $VERSION_TAG https://github.com/DuoSoftware/DVP-ARDSLiteRoutingEngine.git;
fi

cd DVP-ARDSLiteRoutingEngine;
docker build --build-arg VERSION_TAG=$VERSION_TAG -t "ardsliteroutingengine:"$VERSION_TAG .;
cd /usr/src/;
...
if [ ! -d "DVP-DashBoard" ]; then
	git clone -b $VERSION_TAG https://github.com/DuoSoftware/DVP-DashBoard.git
fi

cd DVP-DashBoard;
docker build --build-arg VERSION_TAG=$VERSION_TAG -t "dashboard:"$VERSION_TAG .;
cd /usr/src/;
...
cd /usr/src/;
docker run -d -t --expose=8080/tcp -p 8090:8080 --env="VERSION_TAG=$VERSION_TAG" --env="COMPOSE_DATE=$DATE" --name carrierprovider carrierprovider:$VERSION_TAG asadmin start-domain -v;
;;
