	...
	if (member_session) {
		...
		if (status == SWITCH_STATUS_SUCCESS) {
			...
			if (!globals.rurl){
				...
				char uploaddata[1000];
				switch_snprintf(uploaddata, sizeof(uploaddata), "http://(file=%s.wav)%s/%s/%s?class=CALLSERVER,type=CALL,category=CONVERSATION,referenceid=%s", h->member_uuid, globals.rurl, h->tenant, h->company, h->member_uuid);
